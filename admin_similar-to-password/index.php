<?php header('Content-type=text/html; charset=utf_8'); ?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
<title>Maintain ChatBox entries</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<meta name="Keywords" content="ChatBox, php, script, free, chat, Anonymous">
<meta name="Description" content="ChatBox this chat on pure php without database and java scripts">
<link rel="stylesheet" href="/chat-box/bootstrap.min.css"> 
<link href="data:image/x-icon;base64," rel="icon" type="image/x-icon" />
<style>
<?php include $_SERVER['DOCUMENT_ROOT']."/chat-box/include/style-in-admin.php";?>
</style>
</head>
<body>
<div class="container">
<h1 class="text-center">Maintain ChatBox entries</h1>
<p class="text-center"><a class="btn btn-primary" href="/chat-box/" style="text-decoration: none;" title="View ChatBox">View ChatBox</a> <a href="<?php echo "".$_SERVER['REQUEST_URI'];?>" class="btn btn-primary tt-info hover-tt-bottom" data-hover="refresh page"><span class="glyphicon glyphicon-refresh"></span></a></p>
<p class="text-center"><strong class="text-danger">never delete the last message</strong> in the list Id, otherwise your database will fall(broken), with all messages<br><small class="text-muted">if you still deleted it by accident, then you will have to clear all messages, delete the ChatBox.sqlite file, which is located in this directory</small></p>
<?php

// Maintain ChatBox entries - approve/delete

$id = isset($_GET['id']) ? +$_GET['id'] : 0;
$a  = isset($_GET['a'])  ? $_GET['a']   : '';
try {
  // Open db connection
  $db = new PDO('sqlite:ChatBox.sqlite');
  $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

  //delete entry
  if ( ($a == 'del') && ($id > 0) ) {
    $query = 'DELETE FROM ChatBox WHERE id=:id;';
    $stmt = $db->prepare($query);
    $stmt->bindParam(':id', $id);
    $stmt->execute();
  }

  //approve entry
  if ( ($a == 'app') && ($id > 0) ) {
    $query = 'UPDATE ChatBox SET approved=1 WHERE id=:id;';
    $stmt = $db->prepare($query);
    $stmt->bindParam(':id', $id);
    $stmt->execute();
  }

  $query = 'SELECT * FROM ChatBox ORDER BY id DESC;';
  $result = $db->query($query);
  foreach ( $result as $row ) {
?>
<hr><center><table class="table" style="width:88%;border:none;">
  <tr>
    <td width="120">Id:</td>
    <td><?php echo $row['id']; ?></td>
  </tr>
  <tr>
    <td>Name:</td>
    <td><?php echo $row['name']; ?></td>
  </tr>
  <tr>
    <td valign="top">Message:</td>
    <td><div class="well"><?php echo $row['message']; ?></div></td>
  </tr>
  <tr>
    <td>Date/time:</td>
    <td><?php echo $row['datetime']; ?></td>
  </tr>
  <tr>
    <td><span class="btn btn-default active" style="pointer-events:none;"><?php echo $row['approved'] == 1 ? 'Approved:' : 'Not approved:'; ?></span></td>
    <td class="text-center"><?php if ( $row['approved'] != 1 ) echo '<a class="btn btn-success btn-block" onClick="return confirm(\'Approve entry? Are you sure?\');" href="',htmlspecialchars($_SERVER['PHP_SELF']),'?id=',$row['id'],'&amp;a=app">approve</a>'; ?></td>
  </tr>
  <tr>
    <td><span class="btn btn-default active" style="pointer-events:none;">Delete entry:</span></td>
    <td class="text-center"><a class="btn btn-danger btn-block" onClick="return confirm('Delete entry? Are you sure?');" href="<?php echo htmlspecialchars($_SERVER['PHP_SELF']),'?id=',$row['id']; ?>&amp;a=del">delete</a></td>
  </tr>
</table></center>
<?php
  }
}
catch ( PDOException $e ) {
  echo $e->getMessage();
}
// Close the database
$db = null;
?>
<a href="#top" class="top btn btn-default tt-info hover-tt-right" data-hover="Back to top"><span class="glyphicon glyphicon-hand-up"></span></a>
<br><br><br>
</div>
</body>
</html>
